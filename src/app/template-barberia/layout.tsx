...
export default function TemplateBarberiaLayout({ children }: { children: React.ReactNode }) {
  const [menuOpen, setMenuOpen] = useState(false);

  return (
    <div className="min-h-screen flex flex-col" style={{ fontFamily: "'Inter', sans-serif", backgroundColor: '#131313', color: '#e5e2e1' }}>
      {/* eslint-disable-next-line @next/next/no-page-custom-font */}
      <link href="https://fonts.googleapis.com/css2?family=Newsreader:ital,opsz,wght@0,6..72,400;0,6..72,700;1,6..72,400&family=Inter:wght@400;500;600;700&display=swap" rel="stylesheet" />
      ...
      <header className="fixed top-0 w-full z-50" style={{ background: 'rgba(19,19,19,0.8)', backdropFilter: 'blur(24px)', WebkitBackdropFilter: 'blur(24px)', boxShadow: '0 40px 60px -10px rgba(0,0,0,0.15)' }}>
        <div className="flex justify-between items-center w-full px-6 md:px-8 py-5 max-w-[1920px] mx-auto">
          <Link href="/template-barberia" style={{ fontFamily: "'Newsreader', serif", fontSize: '20px', fontWeight: 700, color: '#e9c176', letterSpacing: '-0.01em', textDecoration: 'none' }}>
            {business.name}
          </Link>
      ...
      {/* Mobile overlay */}
      {menuOpen && <div className="fixed inset-0 z-40 md:hidden" style={{ backgroundColor: 'rgba(0,0,0,0.6)' }} onClick={() => setMenuOpen(false)} />}
      ...
      <footer className="w-full pt-20 pb-12" style={{ backgroundColor: '#0e0e0e' }}>
        <div className="grid grid-cols-1 md:grid-cols-4 gap-12 px-6 md:px-8 max-w-[1920px] mx-auto">
          ...
          <div>
            <h4 style={{ fontSize: '10px', fontWeight: 700, color: '#e9c176', textTransform: 'uppercase', letterSpacing: '0.2em', marginBottom: '20px' }}>Legal</h4>
            <div className="flex flex-col gap-3">
              <span style={{ fontSize: '11px', color: '#9a8f80', textTransform: 'uppercase', letterSpacing: '0.1em' }}>Privacy Policy</span>
              <span style={{ fontSize: '11px', color: '#9a8f80', textTransform: 'uppercase', letterSpacing: '0.1em' }}>Terms of Service</span>
            </div>
          </div>
          <div>
        ...
        <div className="mt-16 px-6 md:px-8 max-w-[1920px] mx-auto flex flex-col md:flex-row justify-between items-center gap-4 pt-8" style={{ borderTop: '1px solid rgba(78,70,57,0.3)' }}>
          <p style={{ fontSize: '10px', color: '#9a8f80', textTransform: 'uppercase', letterSpacing: '0.15em' }}>&copy; {new Date().getFullYear()} {business.legalName}. All rights reserved.{' '}

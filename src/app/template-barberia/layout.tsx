...
export default function TemplateBarberiaLayout(props: any = {}) {
...
const navLinks = Array.isArray(nav) ? nav.map((n: any) => ({ ...n, href: n.href.replace(/^\/template-[^/]+/, baseHref) })) : [];
  ...
  return (
    <div className="min-h-screen flex flex-col" style={{ fontFamily: "'Inter', sans-serif", backgroundColor: '#131313', color: '#e5e2e1' }}>
      ...
      {/* Mobile overlay */}
      {menuOpen && <div className="fixed inset-0 z-[70] md:hidden" style={{ backgroundColor: 'rgba(0,0,0,0.6)' }} onClick={() => setMenuOpen(false)} />}

      {/* Mobile slide-in menu */}
      <div className="fixed top-0 right-0 z-[80] h-full w-72 md:hidden flex flex-col transition-transform duration-300" style={{ backgroundColor: '#0e0e0e', transform: menuOpen ? 'translateX(0)' : 'translateX(100%)', paddingTop: '80px' }}>
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-12 px-6 md:px-8 max-w-[1920px] mx-auto">
          ...
          <div>
            ...
            <div className="flex flex-col gap-3">
              <span style={{ fontSize: '11px', color: '#9a8f80', textTransform: 'uppercase', letterSpacing: '0.1em' }}>Política de privacidad</span>
              <span style={{ fontSize: '11px', color: '#9a8f80', textTransform: 'uppercase', letterSpacing: '0.1em' }}>Términos del servicio</span>
            </div>
          </div>
          <div>

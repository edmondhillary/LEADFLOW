    ...
    <div className="min-h-screen flex flex-col" style={{ fontFamily: "'Manrope', sans-serif", backgroundColor: '#0e0e0e', color: '#ffffff' }}>
      ...
      <header className="fixed top-0 w-full z-50" style={{ background: 'rgba(10,10,10,0.8)', backdropFilter: 'blur(24px)', WebkitBackdropFilter: 'blur(24px)', boxShadow: '0 0 50px rgba(235,0,0,0.08)' }}>
        <div className="flex justify-between items-center px-6 py-4 max-w-full">
          ...
          <a href={`tel:${business_.phoneIntl}`} className="hidden md:inline-block transition-all active:scale-95 hover:bg-red-600" style={{ fontFamily: "'Epilogue', sans-serif", fontSize: '12px', fontWeight: 800, backgroundColor: '#eb0000', color: '#000000', padding: '10px 24px', textDecoration: 'none', textTransform: 'uppercase', letterSpacing: '-0.02em' }}>
            Prueba Gratis
          </a>
      ...
      <div className="fixed top-0 right-0 z-[80] h-full w-72 md:hidden flex flex-col transition-transform duration-300" style={{ backgroundColor: '#131313', transform: menuOpen ? 'translateX(0)' : 'translateX(100%)', paddingTop: '80px', borderLeft: '2px solid #eb0000' }}>
        ...
        <div className="mt-auto px-8 pb-8">
          <a href={`tel:${business_.phoneIntl}`} className="block w-full text-center py-4" style={{ backgroundColor: '#eb0000', color: '#000', fontFamily: "'Epilogue', sans-serif", fontSize: '13px', fontWeight: 900, textTransform: 'uppercase', textDecoration: 'none' }}>
            Prueba Gratis
          </a>
        </div>
      </div>
      ...
      <footer className="w-full py-12 px-6" style={{ backgroundColor: '#0a0a0a', borderTop: '1px solid rgba(255,255,255,0.03)' }}>
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8 max-w-7xl mx-auto">
          <div className="space-y-4">
            <span style={{ fontFamily: "'Epilogue', sans-serif", fontSize: '20px', fontWeight: 900, fontStyle: 'italic', color: '#eb0000', textTransform: 'uppercase' }}>{business_.name}</span>
          ...
          <div className="md:text-right space-y-4">
            <p className="text-sm" style={{ color: '#525252' }}>{business_.email}</p>
            <p className="text-sm" style={{ color: '#525252' }}>{business_.phone}</p>
            <p style={{ fontSize: '10px', color: '#404040', textTransform: 'uppercase', letterSpacing: '0.2em' }}>&copy; {new Date().getFullYear()} {business_.legalName}.{' · '}<a href="https://nexifydev.com" target="_blank" rel="noopener noreferrer" style={{ textDecoration: 'none', opacity: 0.7 }}>Made by Nexifydev.com</a></p>
          </div>
        </div>
      </footer>

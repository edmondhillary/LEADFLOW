...
const navLinks = Array.isArray(nav) ? nav.map((n: any) => ({ ...n, href: n.href.replace(/^\/template-[^/]+/, baseHref) })) : [];
  ...
  return (
    <div className="min-h-screen flex flex-col bg-[#fbf9f5] text-[#30332e]" style={{ fontFamily: "'Manrope', sans-serif" }}>
      {/* eslint-disable-next-line @next/next/no-page-custom-font */}
      <link
        href="https://fonts.googleapis.com/css2?family=Noto+Serif:ital,wght@0,400;0,500;0,600;0,700;1,400;1,600&family=Manrope:wght@300;400;500;600;700&display=swap"
        rel="stylesheet"
      />
      <link
      ...
      />
        {/* Fixed Nav */}
        <header className="fixed top-0 left-0 right-0 z-40 bg-stone-50/80 backdrop-blur-md border-b border-[#b1b3ab]/20">
            ...
            <nav className="hidden md:flex items-center gap-8">
              {navLinks.map((item) => (
                <Link
                  key={item.href}
                  href={item.href}
                  className="text-stone-500 hover:text-stone-800 text-sm tracking-wide transition-colors duration-200"
                >
                  {item.label}
                </Link>
              ))}
          ...
          <nav className="flex flex-col gap-1">
            {navLinks.map((item) => (
              <Link
                ...
                href={item.href}
                onClick={() => setMenuOpen(false)}
                className="flex items-center gap-3 py-3 text-stone-700 hover:text-stone-900 border-b border-[#b1b3ab]/30 text-base tracking-wide"
              >
                <span className="material-symbols-outlined text-[#6c5c4a] text-xl">{item.icon}</span>
                {item.label}
              </Link>
            ))}
          </nav>
        ...
        {/* Footer */}
        <footer className="bg-stone-100 pt-16 pb-8">
          <div className="max-w-7xl mx-auto px-6">
            <div className="grid grid-cols-1 md:grid-cols-4 gap-10 pb-12 border-b border-[#b1b3ab]/30">
              {/* Brand */}
        ...
        <a
          ...
          aria-label="WhatsApp"
          className="fixed bottom-6 right-6 z-50 w-14 h-14 rounded-full bg-[#25d366] flex items-center justify-center shadow-lg hover:bg-[#1ebe5c] transition-colors duration-200"
        >

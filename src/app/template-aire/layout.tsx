...
const navLinks = Array.isArray(nav) ? nav.map((n: any) => ({ ...n, href: n.href.replace(/^\/template-[^/]+/, baseHref) })) : [];
  ...
  return (
    <div className="min-h-screen flex flex-col" style={{ fontFamily: "'Inter', sans-serif", backgroundColor: '#fcf8fb', color: '#1b1b1d' }}>
      ...
      <header className="fixed top-0 w-full z-50 bg-white/70 backdrop-blur-xl shadow-sm">
        <div className="flex justify-between items-center w-full px-6 md:px-8 py-4 max-w-[1920px] mx-auto">
          ...
          <nav className="hidden md:flex items-center gap-8">
            {navLinks.map(n => (
              <Link
                ...
                style={{ fontSize: '13px', fontWeight: 500, color: '#414754', textDecoration: 'none' }}
              >
                {n.label}
              </Link>
            ))}
          ...
          >
            <span className="block w-6 h-[1.5px] transition-all duration-300" style={{ backgroundColor: '#1b1b1d', transform: menuOpen ? 'rotate(45deg) translateY(4.5px)' : 'none' }} />
            <span className="block w-6 h-[1.5px] transition-all duration-300" style={{ backgroundColor: '#1b1b1d', opacity: menuOpen ? 0 : 1 }} />
            <span className="block w-6 h-[1.5px] transition-all duration-300" style={{ backgroundColor: '#1b1b1d', transform: menuOpen ? 'rotate(-45deg) translateY(-4.5px)' : 'none' }} />
          </button>
        </div>
      </header>
        ...
        <nav className="flex flex-col px-8 gap-1">
          {navLinks.map(n => (
            <Link
              ...
              style={{ borderBottom: '1px solid rgba(193,198,215,0.3)', textDecoration: 'none', color: '#1b1b1d' }}
            >
              <span className="material-symbols-outlined" style={{ fontSize: '20px', color: '#8d4b00' }}>{n.icon}</span>
              <span style={{ fontFamily: "'Inter', sans-serif", fontSize: '13px', fontWeight: 500 }}>{n.label}</span>
            </Link>
          ))}
        </nav>
      ...
      {/* Footer */}
      <footer className="w-full pt-20 pb-12" style={{ backgroundColor: '#f6f3f5' }}>
        ...
        >
          ...
          <p style={{ fontSize: '11px', color: '#717786' }}>Austin, TX — NATE Certified</p>
        </div>
      </footer>

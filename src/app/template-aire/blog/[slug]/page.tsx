...
export default async function BlogPostPage({
  ...
  return (
    <article>
      {/* Hero */}
      <section style={{ backgroundColor: '#fcf8fb', padding: '72px 0 0' }}>
        <div style={{ maxWidth: '800px', margin: '0 auto', padding: '0 24px' }}>
          <nav style={{ display: 'flex', gap: '8px', alignItems: 'center', fontSize: '12px', marginBottom: '28px', fontFamily: "'Inter', sans-serif" }}>
            <Link href="/template-aire" style={{ color: '#8d4b00', textDecoration: 'none' }}>Inicio</Link>
            <span style={{ color: '#9ca3af' }}>/</span>
            <Link href="/template-aire/blog" style={{ color: '#8d4b00', textDecoration: 'none' }}>Blog</Link>
            <span style={{ color: '#9ca3af' }}>/</span>
            <span style={{ color: '#9ca3af' }}>{post.title.slice(0, 40)}{post.title.length > 40 ? '...' : ''}</span>
          </nav>

          {'category' in post && (
            <p style={{ fontSize: '11px', fontWeight: 700, color: '#8d4b00', textTransform: 'uppercase', letterSpacing: '0.2em', marginBottom: '16px', fontFamily: "'Inter', sans-serif" }}>
          ...
          <h1 style={{ fontSize: 'clamp(28px, 5vw, 44px)', fontWeight: 800, lineHeight: 1.2, letterSpacing: '-0.02em', color: '#1b1b1d', fontFamily: "'Manrope', sans-serif", marginBottom: '24px' }}>
            {post.title}
          </h1>
      ...
      {/* Body */}
      <section style={{ backgroundColor: '#f0edef', padding: '64px 0 80px' }}>
        <div style={{ maxWidth: '720px', margin: '0 auto', padding: '0 24px' }}>
          {[
            ...
            <p key={i} style={{ fontSize: '17px', lineHeight: 1.85, color: '#1b1b1d', fontFamily: "'Inter', sans-serif", marginBottom: '28px', opacity: i === 0 ? 0.95 : 0.8 }}>
              {para}
            </p>
          ))}

          {/* CTA */}
          <div style={{ borderLeft: '3px solid #8d4b00', paddingLeft: '20px', margin: '48px 0', backgroundColor: '#f6f3f5', padding: '20px 20px 20px 24px', borderRadius: '0 8px 8px 0' }}>
            <p style={{ fontSize: '15px', fontWeight: 600, color: '#1b1b1d', fontFamily: "'Manrope', sans-serif", marginBottom: '8px' }}>
            ...
            <Link href="/template-aire/contacto" style={{ display: 'inline-block', padding: '10px 20px', backgroundColor: '#8d4b00', color: '#ffffff', fontSize: '12px', fontWeight: 700, textDecoration: 'none', borderRadius: '6px', textTransform: 'uppercase', letterSpacing: '0.08em', fontFamily: "'Inter', sans-serif" }}>
              Contactar
            </Link>
          </div>

          <Link href="/template-aire/blog" style={{ display: 'inline-flex', alignItems: 'center', gap: '8px', color: '#8d4b00', fontFamily: "'Inter', sans-serif", fontSize: '14px', fontWeight: 600, textDecoration: 'none' }}>

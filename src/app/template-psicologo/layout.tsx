...
const navLinks = Array.isArray(nav) ? nav.map((n: any) => ({ ...n, href: n.href.replace(/^\/template-[^/]+/, baseHref) })) : [];
  ...
  return (
    <div className="min-h-screen flex flex-col" style={{ fontFamily: "'Manrope', sans-serif", backgroundColor: '#f9f9f7', color: '#1a1c1b' }}>
      ...
      <header className="fixed top-0 w-full z-50" style={{ background: 'rgba(249,249,247,0.8)', backdropFilter: 'blur(12px)', WebkitBackdropFilter: 'blur(12px)', borderBottom: '1px solid rgba(197,199,191,0.3)' }}>
        <div className="flex justify-between items-center w-full px-6 md:px-8 py-5 max-w-[1920px] mx-auto">
          ...
          <nav className="hidden md:flex items-center gap-10">
            {navLinks.map(n => (
              <Link key={n.href} href={n.href} className="hover:text-[#586152] transition-colors duration-300" style={{ fontSize: '11px', fontWeight: 500, color: '#454841', textDecoration: 'none', textTransform: 'uppercase', letterSpacing: '0.15em' }}>
                {n.label}
              </Link>
            ))}
          </nav>
          ...
          <button className="md:hidden flex flex-col justify-center items-center w-10 h-10 gap-1.5" onClick={() => setMenuOpen(!menuOpen)} aria-label="Abrir menú">
            <span className="block w-6 h-[1.5px] transition-all duration-300" style={{ backgroundColor: '#1a1c1b', transform: menuOpen ? 'rotate(45deg) translateY(4.5px)' : 'none' }} />
            <span className="block w-6 h-[1.5px] transition-all duration-300" style={{ backgroundColor: '#1a1c1b', opacity: menuOpen ? 0 : 1 }} />
            <span className="block w-6 h-[1.5px] transition-all duration-300" style={{ backgroundColor: '#1a1c1b', transform: menuOpen ? 'rotate(-45deg) translateY(-4.5px)' : 'none' }} />
          </button>
        </div>
      </header>
      ...
      <div className="fixed top-0 right-0 z-50 h-full w-72 md:hidden flex flex-col transition-transform duration-300" style={{ backgroundColor: '#f9f9f7', transform: menuOpen ? 'translateX(0)' : 'translateX(100%)', paddingTop: '80px' }}>
        <nav className="flex flex-col px-8 gap-1">
          {navLinks.map(n => (
            <Link key={n.href} href={n.href} onClick={() => setMenuOpen(false)} className="flex items-center gap-4 py-4" style={{ borderBottom: '1px solid rgba(88,97,82,0.1)', textDecoration: 'none', color: '#1a1c1b' }}>
              <span className="material-symbols-outlined" style={{ fontSize: '20px', color: '#586152' }}>{n.icon}</span>
        ...
        <div className="mt-auto px-8 pb-8">
          ...
          <p className="mt-4 text-center" style={{ fontSize: '10px', color: '#454841', letterSpacing: '0.1em', textTransform: 'uppercase' }}>{business_.phone}</p>
        </div>
      </div>
      ...
      {/* Footer */}
      <footer className="w-full pt-20 pb-12" style={{ backgroundColor: '#eeeeec' }}>
        <div className="grid grid-cols-1 md:grid-cols-4 gap-12 px-6 md:px-8 max-w-[1920px] mx-auto">
          <div>
            <p style={{ fontFamily: "'Noto Serif', serif", fontSize: '18px', fontStyle: 'italic', color: '#586152', marginBottom: '12px' }}>{business_.name}</p>
            <p style={{ fontFamily: "'Noto Serif', serif", fontSize: '13px', fontStyle: 'italic', color: '#454841', lineHeight: 1.6, marginBottom: '12px' }}>{business_.quote}</p>
            <p style={{ fontSize: '11px', color: '#454841', lineHeight: 1.8, textTransform: 'uppercase', letterSpacing: '0.1em' }}>{business_.tagline}<br />{business_.city}, {business_.country}</p>
          </div>
          <div>
            <h4 style={{ fontSize: '11px', fontWeight: 700, color: '#1a1c1b', textTransform: 'uppercase', letterSpacing: '0.15em', marginBottom: '20px' }}>Navegación</h4>
            <nav className="flex flex-col gap-3">
              {navLinks.map(n => (
                <Link key={n.href} href={n.href} className="hover:text-[#586152] transition-colors duration-300" style={{ fontSize: '11px', color: '#454841', textDecoration: 'none', textTransform: 'uppercase', letterSpacing: '0.1em' }}>{n.label}</Link>
              ))}
            </nav>
          </div>
          <div>
            <h4 style={{ fontSize: '11px', fontWeight: 700, color: '#1a1c1b', textTransform: 'uppercase', letterSpacing: '0.15em', marginBottom: '20px' }}>Contacto</h4>
            <div style={{ fontSize: '11px', color: '#454841', lineHeight: 2, textTransform: 'uppercase', letterSpacing: '0.1em' }}>
              <p>{business_.email}</p>
              <p>{business_.phone}</p>
              <p>{business_.address}</p>
              <p>{business_.postalCode} {business_.city}</p>
            </div>
          </div>
          <div>
            <h4 style={{ fontSize: '11px', fontWeight: 700, color: '#1a1c1b', textTransform: 'uppercase', letterSpacing: '0.15em', marginBottom: '20px' }}>Legal</h4>
            <div className="flex flex-col gap-3">
              <span style={{ fontSize: '11px', color: '#454841', textTransform: 'uppercase', letterSpacing: '0.1em' }}>Política de Privacidad</span>
              <span style={{ fontSize: '11px', color: '#454841', textTransform: 'uppercase', letterSpacing: '0.1em' }}>Aviso Legal</span>
              <span style={{ fontSize: '11px', color: '#454841', textTransform: 'uppercase', letterSpacing: '0.1em' }}>Política de Cookies</span>
            </div>
          </div>
        </div>
        <div className="mt-16 px-6 md:px-8 max-w-[1920px] mx-auto flex flex-col md:flex-row justify-between items-center gap-4 pt-8" style={{ borderTop: '1px solid rgba(88,97,82,0.15)' }}>
          <p style={{ fontSize: '10px', color: '#454841', textTransform: 'uppercase', letterSpacing: '0.15em' }}>
          ...
          <p style={{ fontSize: '10px', color: '#454841', textTransform: 'uppercase', letterSpacing: '0.15em', fontStyle: 'italic' }}>Diseñado con cuidado en Madrid</p>
        </div>
      </footer>

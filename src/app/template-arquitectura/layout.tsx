    ...
    <div className="min-h-screen flex flex-col" style={{ fontFamily: "'Manrope', sans-serif", backgroundColor: '#f9f9f6', color: '#2e3430' }}>
      ...
      <header className="fixed top-0 w-full z-50" style={{ background: 'rgba(250,247,243,0.8)', backdropFilter: 'blur(16px)', WebkitBackdropFilter: 'blur(16px)' }}>
        <div className="flex justify-between items-center w-full px-6 md:px-8 py-5 max-w-[1920px] mx-auto">
          ...
          <a href={`tel:${business_.phoneIntl}`} className="hidden md:inline-block transition-all active:scale-[0.98]" style={{ fontSize: '11px', fontWeight: 600, backgroundColor: '#5f5e5e', color: '#faf7f6', padding: '12px 28px', textDecoration: 'none', textTransform: 'uppercase', letterSpacing: '0.15em' }}>
            Solicitar Proyecto
          </a>
      ...
      <div className="fixed top-0 right-0 z-[80] h-full w-72 md:hidden flex flex-col transition-transform duration-300" style={{ backgroundColor: '#f9f9f6', transform: menuOpen ? 'translateX(0)' : 'translateX(100%)', paddingTop: '80px' }}>
        ...
        <div className="mt-auto px-8 pb-8">
          <a href={`tel:${business_.phoneIntl}`} className="block w-full text-center py-4" style={{ backgroundColor: '#5f5e5e', color: '#faf7f6', fontSize: '11px', fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.15em', textDecoration: 'none' }}>
            Llamar Ahora
          </a>
          <p className="mt-4 text-center" style={{ fontSize: '10px', color: '#635e57', letterSpacing: '0.1em', textTransform: 'uppercase' }}>{business_.phone}</p>
        </div>
      </div>
      ...
      <footer className="w-full pt-20 pb-12" style={{ backgroundColor: '#f3f4f0' }}>
        <div className="grid grid-cols-1 md:grid-cols-4 gap-12 px-6 md:px-8 max-w-[1920px] mx-auto">
          <div>
            <p style={{ fontFamily: "'Noto Serif', serif", fontSize: '18px', fontStyle: 'italic', color: '#2e3430', marginBottom: '16px' }}>{business_.name}</p>
            <p style={{ fontSize: '11px', color: '#635e57', lineHeight: 1.8, textTransform: 'uppercase', letterSpacing: '0.1em' }}>{business_.tagline}<br />{business_.city}, {business_.country}</p>
          </div>
          <div>
        ...
        <div className="mt-16 px-6 md:px-8 max-w-[1920px] mx-auto flex flex-col md:flex-row justify-between items-center gap-4 pt-8" style={{ borderTop: '1px solid rgba(118,124,119,0.15)' }}>
          <p style={{ fontSize: '10px', color: '#635e57', textTransform: 'uppercase', letterSpacing: '0.15em' }}>&copy; {new Date().getFullYear()} {business_.legalName}. Todos los derechos reservados.{' · '}<a href="https://nexifydev.com" target="_blank" rel="noopener noreferrer" style={{ textDecoration: 'none', opacity: 0.7 }}>Made by Nexifydev.com</a></p>

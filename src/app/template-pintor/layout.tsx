    ...
    <div className="min-h-screen flex flex-col bg-[#f9f9f9] text-[#2d3435]" style={{ fontFamily: "'Work Sans', sans-serif" }}>
      ...
      <header className="fixed top-0 w-full z-50 bg-white/80 backdrop-blur-xl shadow-sm">
        <div className="flex justify-between items-center px-6 py-4 max-w-7xl mx-auto">
          ...
          <a href={`tel:${business.phoneIntl}`} className="hidden md:inline-block bg-[#3d6565] hover:bg-[#315959] text-[#d9fffe] px-5 py-2.5 rounded-md font-medium text-sm transition-all duration-300 active:scale-95 shadow-sm" style={{ textDecoration: 'none' }}>
            Pedir Presupuesto
          </a>
      ...
      {/* Mobile overlay */}
      {menuOpen && <div className="fixed inset-0 z-40 md:hidden bg-black/20" onClick={() => setMenuOpen(false)} />}

      {/* Mobile slide-in */}
      <div className="fixed top-0 right-0 z-50 h-full w-72 md:hidden flex flex-col bg-[#f9f9f9] shadow-2xl transition-transform duration-300" style={{ transform: menuOpen ? 'translateX(0)' : 'translateX(100%)', paddingTop: '80px' }}>
        ...
        <div className="mt-auto px-6 pb-8">
          <a href={`tel:${business.phoneIntl}`} className="block w-full text-center py-3 rounded-md text-sm font-semibold" style={{ background: 'linear-gradient(135deg, #3d6565, #315959)', color: '#d9fffe', textDecoration: 'none' }}>
            Pedir Presupuesto
          </a>
        </div>
      </div>
      ...
      <footer className="bg-zinc-100 w-full" style={{ borderTop: '1px solid rgba(228,233,234,0.5)' }}>
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8 px-6 md:px-10 py-16 max-w-7xl mx-auto">
          <div className="space-y-4">
            <span className="font-bold text-teal-800 text-lg" style={{ fontFamily: "'Manrope', sans-serif" }}>{business.name}</span>
          ...
          <div className="flex flex-col gap-3">
            ...
            <p className="text-zinc-500 text-sm flex items-center gap-2">
              <span className="material-symbols-outlined text-sm">mail</span> {business.email}
            </p>
            <p className="text-zinc-500 text-sm flex items-center gap-2">
              <span className="material-symbols-outlined text-sm">call</span> {business.phone}
            </p>
            <p className="text-zinc-500 text-sm flex items-center gap-2">
              <span className="material-symbols-outlined text-sm">location_on</span> {business.city}, {business.country}
            </p>
          </div>
        </div>
        <div className="px-10 py-6 text-center" style={{ borderTop: '1px solid rgba(228,233,234,0.3)' }}>
          <p className="text-zinc-500 text-xs">&copy; {new Date().getFullYear()} {business.legalName}. Todos los derechos reservados.{' · '}<a href="https://nexifydev.com" target="_blank" rel="noopener noreferrer" style={{ textDecoration: 'none', opacity: 0.7 }}>Made by Nexifydev.com</a></p>
        </div>
      </footer>

      {/* WhatsApp FAB */}
      <a href={`https://wa.me/${business.whatsapp}`} target="_blank" rel="noopener noreferrer" aria-label="Contactar por WhatsApp" className="fixed bottom-6 right-6 z-50 w-14 h-14 rounded-full shadow-[0_12px_32px_rgba(45,52,53,0.15)] flex items-center justify-center transition-all hover:scale-105" style={{ backgroundColor: '#25d366' }}>

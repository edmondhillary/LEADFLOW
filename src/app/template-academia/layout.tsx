...
const navLinks = Array.isArray(nav) ? nav.map((n: any) => ({ ...n, href: n.href.replace(/^\/template-[^/]+/, baseHref) })) : [];
  ...
  return (
    <div className="min-h-screen flex flex-col" style={{ fontFamily: "'Inter', sans-serif", backgroundColor: '#f8f9fa', color: '#191c1d' }}>
      ...
      {/* Mobile overlay */}
      {menuOpen && <div className="fixed inset-0 z-[70] md:hidden" style={{ backgroundColor: 'rgba(0,0,0,0.4)' }} onClick={() => setMenuOpen(false)} />}
      ...
      <div className="fixed top-0 right-0 z-[80] h-full w-72 md:hidden flex flex-col transition-transform duration-300" style={{ backgroundColor: '#ffffff', transform: menuOpen ? 'translateX(0)' : 'translateX(100%)', paddingTop: '80px' }}>
        ...
        <div className="mt-auto px-8 pb-8 flex flex-col gap-3">
          ...
          <p className="text-center" style={{ fontSize: '12px', color: '#454652' }}>{business_.phone}</p>
        </div>
      </div>
      ...
      <footer className="w-full pt-20 pb-12" style={{ backgroundColor: '#001944' }}>
        <div className="grid grid-cols-1 md:grid-cols-4 gap-12 px-6 md:px-10 max-w-[1440px] mx-auto">
          {/* Brand + social */}
          <div>
            <p style={{ fontFamily: "'Manrope', sans-serif", fontSize: '20px', fontWeight: 800, color: '#ffffff', letterSpacing: '-0.04em', marginBottom: '12px' }}>{business_.name}</p>
            <p style={{ fontSize: '13px', color: '#6b95f3', lineHeight: 1.8, marginBottom: '20px' }}>{business_.tagline}</p>
            <p style={{ fontSize: '13px', color: 'rgba(255,255,255,0.5)', lineHeight: 1.7 }}>{business_.address}<br />{business_.postalCode} {business_.city}</p>
        ...
        <div className="mt-16 px-6 md:px-10 max-w-[1440px] mx-auto flex flex-col md:flex-row justify-between items-center gap-4 pt-8" style={{ borderTop: '1px solid rgba(255,255,255,0.08)' }}>
          <p style={{ fontSize: '12px', color: 'rgba(255,255,255,0.35)', letterSpacing: '0.05em' }}>&copy; {new Date().getFullYear()} {business_.legalName}. Todos los derechos reservados.{' '}
